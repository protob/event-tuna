...
export default {
	...
	setLoading({ commit, rootState }, isLoading) {

		// console.log('loading', rootState)

		commit('SET_LOADING_STATE', { isLoading }, { root: true })
		// rootState.commit('SET_LOADING_STATE', {
		//   isLoading: true
		// })
	},
	registerUserWithEmailAndPassword({ dispatch, commit }, form) {
		return firebase
			.auth()
			...
			setTimeout(() => {
				ref.get().then((docSnapshot) => {
					if (docSnapshot.exists) {
						...
						// this.$root.$emit('userHasLoggenIn', true)
						// ref.onSnapshot(doc => {})
					} else {
						console.error('error')
						resolve(null)
